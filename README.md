...
| Bignum exponentiation policy | One binary exponentiation helper served both public and secret exponents. | Keep the existing variable-time Montgomery path for public/non-secret values such as Miller–Rabin and RSA `e`, and route private/random exponents through the separate fixed-schedule path. | Allows future aggressive public-data optimization without silently weakening secret-exponent timing behavior. |
...
| Secret temporary storage | Secret exponent state used ordinary variable-length bignums and generic temporaries. | Use modulus-width limb buffers for secret Montgomery state, mask-select candidate results, fixed-count output normalization, and explicit zeroization before release. | Reduce secret-dependent control flow and residual secret intermediates. |
