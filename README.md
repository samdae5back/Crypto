...
| Bignum Montgomery exponentiation | Allocate and free the `n + 2`-limb Montgomery scratch array for every Montgomery multiplication. | Allocate one scratch array in the per-exponentiation Montgomery context, clear and reuse it for every multiply, then securely clear it when the context is released. | Removes one heap allocation/free pair from every Montgomery multiplication used by ElGamal, RSA, and Miller–Rabin exponentiation while also reducing residual intermediate data. |
