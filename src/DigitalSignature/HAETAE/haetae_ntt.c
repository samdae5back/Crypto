...
static const int zetas_haetae[CRYPTO_HAETAE_N] = {
...
void crypto_haetae_ntt(int a[CRYPTO_HAETAE_N]) {
    ...
    for (len = 128; len > 0; len >>= 1) {
        for (start = 0; start < CRYPTO_HAETAE_N; start = j + len) {
            zeta = zetas_haetae[++k];
            for (j = start; j < start + len; ++j) {
                t = crypto_haetae_montgomery_reduce((long long)zeta * a[j + len]);
                a[j + len] = a[j] - t;
                a[j] = a[j] + t;
            }
        }
...
void crypto_haetae_invntt_tomont(int a[CRYPTO_HAETAE_N]) {
    ...
    int t, zeta;
    const int f = -29720; // mont^2/256

    k = 256;
    for (len = 1; len < CRYPTO_HAETAE_N; len <<= 1) {
        for (start = 0; start < CRYPTO_HAETAE_N; start = j + len) {
            zeta = -zetas_haetae[--k];
            for (j = start; j < start + len; ++j) {
                t = a[j];
                a[j] = t + a[j + len];
                a[j + len] = t - a[j + len];
                a[j + len] = crypto_haetae_montgomery_reduce((long long)zeta * a[j + len]);
            }
        }
    }

    for (j = 0; j < CRYPTO_HAETAE_N; ++j) {
        a[j] = crypto_haetae_montgomery_reduce((long long)f * a[j]);
    }
}
