...
static inline int RansDecInit(
    RansState* r, const uint8_t** pptr, const uint8_t* end)
{
    uint32_t x;
...
static inline void RansDecAdvance(RansState* r, const uint8_t** pptr, const uint8_t *end, uint32_t start, uint32_t freq, uint32_t scale_bits)
{
...
static inline void RansEncSymbolInit(RansEncSymbol* s, uint32_t start, uint32_t freq, uint32_t scale_bits)
{
    ...
    s->cmpl_freq =
        (uint16_t)((UINT32_C(1) << scale_bits) - freq);
    if (freq < 2) {
        ...
        s->rcp_shift = 0;
        s->bias = start + (UINT32_C(1) << scale_bits) - 1;
    } else {
...
{
    RansAssert(start <= (UINT32_C(1) << 16));
    RansAssert(freq <= (UINT32_C(1) << 16) - start);
...
static inline void RansEncPutSymbol(RansState* r, uint8_t** pptr, RansEncSymbol const* sym)
{
    ...
    q = (uint32_t) (((uint64_t)x * sym->rcp_freq) >> 32) >> sym->rcp_shift;
    *r = x + sym->bias + q * sym->cmpl_freq;
}

// Equivalent to RansDecAdvance that takes a symbol.
static inline void RansDecAdvanceSymbol(RansState* r, const uint8_t** pptr, const uint8_t *end, RansDecSymbol const* sym, uint32_t scale_bits)
{
