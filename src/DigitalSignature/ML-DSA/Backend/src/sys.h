...
/* This project builds the portable C backend only, so no assembly CET note is
 * emitted and no compiler-specific <cet.h> dependency is needed. */
#define MLD_CET_ENDBR
#endif /* MLD_SYS_X86_64 */

/* Runtime Valgrind annotations are intentionally not part of the bundled
 * implementation; production constant-time behavior does not depend on them. */
#if defined(MLD_CONFIG_CT_TESTING_ENABLED)
#error MLD_CONFIG_CT_TESTING_ENABLED requires an out-of-tree test adapter
#endif
