...
size_t LIBERAC_ML_KEM_PUBLIC_KEY_SIZE(LiberaCAlgID alg) {
    return crypto_ml_kem_public_key_size_internal(alg);
}

size_t LIBERAC_ML_KEM_PRIVATE_KEY_SIZE(LiberaCAlgID alg) {
    return crypto_ml_kem_private_key_size_internal(alg);
}

size_t LIBERAC_ML_KEM_CIPHERTEXT_SIZE(LiberaCAlgID alg) {
    return crypto_ml_kem_ciphertext_size_internal(alg);
}

LiberaCError LIBERAC_ML_KEM_KEYGEN(uint8_t *public_key, size_t public_key_length,
...
LiberaCError LIBERAC_ML_KEM_ENCAPS(
    const uint8_t *public_key, size_t public_key_length,
    uint8_t shared_secret[LIBERAC_ML_KEM_SHARED_SECRET_BYTES],
...
LiberaCError LIBERAC_ML_KEM_DECAPS(
...
size_t LIBERAC_NTRU_PLUS_PUBLIC_KEY_SIZE(LiberaCAlgID alg) {
    return crypto_ntru_plus_public_key_size_internal(alg);
}

size_t LIBERAC_NTRU_PLUS_PRIVATE_KEY_SIZE(LiberaCAlgID alg) {
    return crypto_ntru_plus_private_key_size_internal(alg);
}

size_t LIBERAC_NTRU_PLUS_CIPHERTEXT_SIZE(LiberaCAlgID alg) {
    return crypto_ntru_plus_ciphertext_size_internal(alg);
}

LiberaCError LIBERAC_NTRU_PLUS_KEYGEN(
...
LiberaCError LIBERAC_NTRU_PLUS_ENCAPS(
    const uint8_t *public_key, size_t public_key_length,
...
LiberaCError LIBERAC_NTRU_PLUS_DECAPS(
    const uint8_t *private_key, size_t private_key_length,
    const uint8_t *ciphertext, size_t ciphertext_length,
    uint8_t shared_secret[LIBERAC_NTRU_PLUS_SHARED_SECRET_BYTES],
    LiberaCAlgID alg) {
...
size_t LIBERAC_SMAUG_T_PUBLIC_KEY_SIZE(LiberaCAlgID alg) {
    return crypto_smaug_t_public_key_size_internal(alg);
}

size_t LIBERAC_SMAUG_T_PRIVATE_KEY_SIZE(LiberaCAlgID alg) {
    return crypto_smaug_t_private_key_size_internal(alg);
}

size_t LIBERAC_SMAUG_T_CIPHERTEXT_SIZE(LiberaCAlgID alg) {
    return crypto_smaug_t_ciphertext_size_internal(alg);
}

LiberaCError LIBERAC_SMAUG_T_KEYGEN(
    uint8_t *public_key, size_t public_key_length,
    uint8_t *private_key, size_t private_key_length,
    LiberaCAlgID alg) {
...
LiberaCError LIBERAC_SMAUG_T_ENCAPS(
    const uint8_t *public_key, size_t public_key_length,
...
LiberaCError LIBERAC_SMAUG_T_DECAPS(
    const uint8_t *private_key, size_t private_key_length,
    const uint8_t *ciphertext, size_t ciphertext_length,
    uint8_t shared_secret[LIBERAC_SMAUG_T_SHARED_SECRET_BYTES],
    LiberaCAlgID alg) {
