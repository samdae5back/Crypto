...
#include "hash.h"
#include "RANDOM.h"
void RBG(unsigned char *seed,size_t len){if(CRYPTO_RANDOM_BYTES(seed,len)!=0){fprintf(stderr,"RBG failed: operating-system CSPRNG unavailable\n");exit(EXIT_FAILURE);}}
...
void G(unsigned char *in,size_t n,unsigned char *o1,unsigned char *o2){unsigned char out[64];CRYPTO_SHA3_512(out,in,n);memcpy(o1,out,32);memcpy(o2,out+32,32);memset(out,0,sizeof(out));}
