...
`LIBERAC_ALG_SMAUG_T_128`, `LIBERAC_ALG_SMAUG_T_192`, and
`LIBERAC_ALG_SMAUG_T_256` identifiers. Upstream Keccak and random-byte
implementations are not included. The port uses LiberaCrypt's private shared
SHA3/SHAKE, random-byte, endian, constant-time, and secure-zeroization utilities
instead. Internal non-static symbols use the `crypto_smaug_t_` namespace.
