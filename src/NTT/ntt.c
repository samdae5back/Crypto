...
int NTT_FORWARD(const NTT_PLAN *p,uint32_t *a){return transform(p,a,p->ROOT);}
int NTT_INVERSE(const NTT_PLAN *p,uint32_t *a){size_t i;if(transform(p,a,p->ROOT_INV))return -1;for(i=0;i<p->N;++i)a[i]=(uint32_t)(((uint64_t)a[i]*p->N_INV)%p->MODULUS);return 0;}
