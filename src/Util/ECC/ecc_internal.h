...
/*
 * Internal ECC interfaces are not part of the installed ABI.  Field values use
 * fixed 32-bit Montgomery limbs; unused high limbs must remain zero.
 */
...
/*
 * Textbook affine binary multiplication.  This is a variable-time review and
 * differential-test baseline; never use it with a private scalar.
 */
...
/*
 * Optimized Jacobian four-bit window for public scalars.  Branches and direct
 * table indices depend on scalar digits by design.
 */
LiberaCError crypto_ec_scalar_multiply_vartime(
    ...
    size_t scalar_length);
/*
 * Fixed-width Montgomery ladder for private scalars.  The software schedule
 * contains one complete-behavior add, one double, and masked swaps per bit.
 */
LiberaCError crypto_ec_scalar_multiply_ct(
    const CryptoEcCurve *curve, CryptoEcAffinePoint *out,
    const CryptoEcAffinePoint *point, const uint8_t *scalar,
    size_t scalar_length);
/* Fixed-width check for 1 <= scalar < curve order. */
