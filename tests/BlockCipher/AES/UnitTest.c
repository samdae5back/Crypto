...
static int key_size_test(void) {
    ...
    return LIBERAC_BLOCK_CIPHER_KEY_SIZE(LIBERAC_ALG_NONE) != 0u ||
           ...
           LIBERAC_BLOCK_CIPHER_KEY_SIZE(LIBERAC_ALG_AES_128_GCM) != 0u ||
           LIBERAC_BLOCK_CIPHER_KEY_SIZE(LIBERAC_ALG_AES_128_CCM) != 0u;
}
...
static int all_parameter_sets_roundtrip(void) {
    ...
    for (j = 0u;
        ...
        if (alg == LIBERAC_ALG_AES_128_CTR || alg == LIBERAC_ALG_AES_192_CTR ||
            ...
            mode_iv = iv;
            iv_length = sizeof(iv);
        } else if (alg == LIBERAC_ALG_AES_128_CBC ||
                   alg == LIBERAC_ALG_AES_192_CBC ||
        ...
        if (LIBERAC_BLOCK_CIPHER_ENCRYPT(
                ciphertext, sizeof(ciphertext), plaintext, input_length,
                key, AES_PARAMETER_SETS[j].KEY_LENGTH,
                mode_iv, iv_length, alg) != LIBERAC_SUCCESS ||
            LIBERAC_BLOCK_CIPHER_DECRYPT(
                recovered, sizeof(recovered), ciphertext, input_length,
                key, AES_PARAMETER_SETS[j].KEY_LENGTH,
                mode_iv, iv_length, alg) != LIBERAC_SUCCESS ||
            memcmp(recovered, plaintext, input_length) != 0) {
            return 1;
        }
    }
    return 0;
...
static int negative_test(void) {
    ...
    return LIBERAC_BLOCK_CIPHER_ENCRYPT(
               output, sizeof(output), input, 16u, key, sizeof(key),
               NULL, 0u, LIBERAC_ALG_NONE) !=
               LIBERAC_ERROR_INVALID_ALG_ID ||
           LIBERAC_BLOCK_CIPHER_ENCRYPT(
               output, sizeof(output), input, 16u, key, sizeof(key),
               NULL, 0u, (LiberaCAlgID)0x00500010) !=
               LIBERAC_ERROR_INVALID_ALG_ID ||
           LIBERAC_BLOCK_CIPHER_ENCRYPT(
               output, sizeof(output), input, 16u, key, sizeof(key) - 1u,
               NULL, 0u, LIBERAC_ALG_AES_128_ECB) !=
               LIBERAC_ERROR_INVALID_KEY ||
           LIBERAC_BLOCK_CIPHER_ENCRYPT(
               output, sizeof(output), input, 17u, key, sizeof(key),
               iv, sizeof(iv), LIBERAC_ALG_AES_128_CBC) !=
               LIBERAC_ERROR_INVALID_ARGUMENT ||
           LIBERAC_BLOCK_CIPHER_ENCRYPT(
               output, 15u, input, 16u, key, sizeof(key),
               NULL, 0u, LIBERAC_ALG_AES_128_ECB) !=
               LIBERAC_ERROR_BUFFER_TOO_SMALL ||
           LIBERAC_BLOCK_CIPHER_ENCRYPT(
               output, sizeof(output), input, 16u, key, sizeof(key),
               iv, 12u, LIBERAC_ALG_AES_128_GCM) !=
               LIBERAC_ERROR_INVALID_ALG_ID ||
           LIBERAC_BLOCK_CIPHER_ENCRYPT(
               output, sizeof(output), input, 16u, key, sizeof(key),
               iv, 13u, LIBERAC_ALG_AES_128_CCM) !=
               LIBERAC_ERROR_INVALID_ALG_ID;
}
...
int main(void) {
    ...
    if (all_parameter_sets_roundtrip()) {
        fputs("AES block-mode roundtrip failed\n", stderr);
        return 1;
    }
    if (negative_test()) {
