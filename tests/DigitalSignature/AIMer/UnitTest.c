...
static const AlgID algorithms[] = {
    ALG_AIMER_128F, ALG_AIMER_128S,
    ALG_AIMER_192F, ALG_AIMER_192S,
    ALG_AIMER_256F, ALG_AIMER_256S
};
...
static int run_case(AlgID alg) {
    ...
    const size_t public_key_length = CRYPTO_AIMER_PUBLIC_KEY_SIZE(alg);
    const size_t private_key_length = CRYPTO_AIMER_PRIVATE_KEY_SIZE(alg);
    const size_t signature_length = CRYPTO_AIMER_SIGNATURE_SIZE(alg);
    ...
    if (error != CRYPTO_ERROR_INVALID_ARGUMENT) {
    ...
    error = CRYPTO_AIMER_VERIFY(
        ...
        context, sizeof(context) - 1u,
        signature, signature_length, alg);
    if (error != CRYPTO_ERROR_SIGNATURE_INVALID) {
...
int main(void) {
    ...
    if (CRYPTO_AIMER_PUBLIC_KEY_SIZE(ALG_NONE) != 0u ||
        CRYPTO_AIMER_PRIVATE_KEY_SIZE(ALG_NONE) != 0u ||
        CRYPTO_AIMER_SIGNATURE_SIZE(ALG_NONE) != 0u) {
        return 1;
    }
