...
static const AlgID algorithms[] = {
    ALG_HAETAE_120, ALG_HAETAE_180, ALG_HAETAE_260
};
...
static int run_case(AlgID alg) {
    ...
    const size_t public_key_length = CRYPTO_HAETAE_PUBLIC_KEY_SIZE(alg);
    const size_t private_key_length = CRYPTO_HAETAE_PRIVATE_KEY_SIZE(alg);
    const size_t signature_length = CRYPTO_HAETAE_SIGNATURE_SIZE(alg);
    ...
    error = CRYPTO_HAETAE_VERIFY(
        ...
        context, sizeof(context) - 1u,
        signature, signature_length, alg);
    if (error != CRYPTO_ERROR_SIGNATURE_INVALID) goto cleanup;
    result = 0;
...
int main(void) {
    ...
    if (CRYPTO_HAETAE_PUBLIC_KEY_SIZE(ALG_NONE) != 0u ||
        CRYPTO_HAETAE_PRIVATE_KEY_SIZE(ALG_NONE) != 0u ||
        CRYPTO_HAETAE_SIGNATURE_SIZE(ALG_NONE) != 0u) {
        return 1;
    }
