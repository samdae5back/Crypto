...
int crypto_test_run_kem_kat(
    ...
    if (path == NULL || algorithm_name == NULL || api == NULL ||
        api->shared_secret_size == 0u || api->public_key_size == NULL ||
        api->private_key_size == NULL || api->ciphertext_size == NULL ||
        api->keygen == NULL || api->encaps == NULL || api->decaps == NULL) {
        return 2;
    }

    public_key_length = api->public_key_size(alg);
    private_key_length = api->private_key_size(alg);
    ciphertext_length = api->ciphertext_size(alg);
    if (public_key_length == 0u || private_key_length == 0u ||
        ciphertext_length == 0u) {
        fprintf(stderr, "%s: unsupported KEM algorithm\n", algorithm_name);
        return 2;
    }
    ...
    expected_shared_secret = (uint8_t *)malloc(api->shared_secret_size);
    ...
    encapsulated_secret = (uint8_t *)malloc(api->shared_secret_size);
    decapsulated_secret = (uint8_t *)malloc(api->shared_secret_size);
    if (expected_public_key == NULL || expected_private_key == NULL ||
        expected_ciphertext == NULL || expected_shared_secret == NULL ||
        actual_public_key == NULL || actual_private_key == NULL ||
        actual_ciphertext == NULL || encapsulated_secret == NULL ||
        decapsulated_secret == NULL) {
...
cleanup:
    ...
    if (expected_shared_secret != NULL)
        crypto_zeroize(expected_shared_secret, api->shared_secret_size);
    ...
    if (encapsulated_secret != NULL)
        crypto_zeroize(encapsulated_secret, api->shared_secret_size);
    if (decapsulated_secret != NULL)
        crypto_zeroize(decapsulated_secret, api->shared_secret_size);
    free(decapsulated_secret);
    free(encapsulated_secret);
    free(actual_ciphertext);
    free(actual_private_key);
    free(actual_public_key);
    free(expected_shared_secret);
