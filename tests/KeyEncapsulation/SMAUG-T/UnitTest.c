...
static const AlgID algorithms[] = {
    ALG_SMAUG_T_128, ALG_SMAUG_T_192, ALG_SMAUG_T_256
};
...
static int run_case(AlgID alg) {
    ...
    uint8_t encapsulated[CRYPTO_SMAUG_T_SHARED_SECRET_BYTES];
    uint8_t decapsulated[CRYPTO_SMAUG_T_SHARED_SECRET_BYTES];
    CryptoError error;
    int result = 1;

    if (public_key == NULL || private_key == NULL || ciphertext == NULL)
        goto cleanup;
    error = CRYPTO_SMAUG_T_KEYGEN(
        public_key, public_key_length, private_key, private_key_length, alg);
    if (error != CRYPTO_SUCCESS) goto cleanup;
    error = CRYPTO_SMAUG_T_ENCAPS(
...
int main(void) {
    ...
    if (CRYPTO_SMAUG_T_PUBLIC_KEY_SIZE(ALG_NONE) != 0u ||
        CRYPTO_SMAUG_T_PRIVATE_KEY_SIZE(ALG_NONE) != 0u ||
        CRYPTO_SMAUG_T_CIPHERTEXT_SIZE(ALG_NONE) != 0u) {
        return 1;
    }
