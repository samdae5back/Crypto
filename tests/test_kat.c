...
static int aes_kat(void) {
    static const struct {
        ...
        { ALG_AES_128, 16u, {0x3a,0xd7,0x8e,0x72,0x6c,0x1e,0xc0,0x2b,0x7e,0xbf,0xe9,0x2b,0x23,0xd9,0xec,0x34} },
        { ALG_AES_192, 24u, {0x6c,0xd0,0x25,0x13,0xe8,0xd4,0xdc,0x98,0x6b,0x4a,0xfe,0x08,0x7a,0x60,0xbd,0x0c} },
        { ALG_AES_256, 32u, {0xdd,0xc6,0xbf,0x79,0x0c,0x15,0x76,0x0d,0x8d,0x9a,0xeb,0x6f,0x9a,0x75,0xfd,0x4e} }
    };
    uint8_t key[32] = {0};
    ...
    for (i = 0; i < sizeof(vectors) / sizeof(vectors[0]); ++i) {
        if (CRYPTO_AES_ECB_ENCRYPT(vectors[i].alg, key, vectors[i].key_length,
                                   plaintext, sizeof(plaintext), ciphertext,
                                   sizeof(ciphertext)) != CRYPTO_SUCCESS)
            return 1;
...
static int sha3_kat(void) {
    ...
    uint8_t digest[32];
    CRYPTO_SHA3_256(digest, NULL, 0u);
